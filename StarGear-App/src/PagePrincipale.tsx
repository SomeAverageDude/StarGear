...
const jeuxDuMomentIDs = [1877,1942,11133,26226,  119171, 1020, 1802, 9630, 114283 ];
export default function PagePrincipale() {
  const [jeux, setJeux] = useState<JeuIGDB[]>([]);
  const [jeuxDuMoment, setJeuxDuMoment] = useState<JeuIGDB[]>([]);
  // const [loading, setloading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {

    fetch(`http://localhost:4000/igdb/jeux`)
      .then((r) => r.json())
      .then((data) => {
      ...
      .catch(console.error);
       ...
       fetch(`http://localhost:4000/igdb/jeux?ids=${jeuxDuMomentIDs}`)
      .then((r) => r.json())
      .then((data) => setJeuxDuMoment(Array.isArray(data) ? data : []))
      .catch(console.error);
  ...
  return (
    <div className="bg-black text-white min-vh-100">
              ...
              >
                <div className="card bg-transparent border-0 shadow-lg hover-zoom">
                  ...
                  <div
                    ...
                    style={{ fontSize: "10px" }}
                  >
                    <span className="d-block text-truncate">{j.nom}</span>
      ...
      {/* ── Jeux du Moment ── */}
      <section
        className="container my-5 py-5 rounded-4"
        style={{ background: "linear-gradient(180deg, #1a1a1a, #000)" }}
      >
        <h2 className="text-center fw-bold mb-5">Jeux du Moment</h2>
        <div className="row g-4">
          <div className="col-lg-5">
            {jeuxDuMoment.slice(0, 5).map((j) => (
              <div
                key={j.igdb_id}
